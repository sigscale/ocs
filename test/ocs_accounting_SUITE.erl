...
init_per_suite(Config) ->
	...
	Username = "25252525",
	Config1 = [{username, Username} | Config],
	Password = ocs:generate_password(),
	Config2 = [{password, Password} | Config1],
	ok = ocs:add_subscriber(Username, Password, [], 1000),
...
radius_accouting(Config) ->
	...
	A2 = radius_attributes:add(?UserPassword, Password, A1),
	A3 = radius_attributes:add(?NasIdentifier, NasId, A2),
	A4 = radius_attributes:add(?NasPortType, 19, A3),
	A5 = radius_attributes:add(?CallingStationId,"de:ad:be:ef:ca:fe", A4),
	A6 = radius_attributes:add(?AcctSessionId, AcctSessionId, A5),
	AccessRequest = #radius{code = ?AccessRequest, id = Id,
	...
	NextId = Id + 1,
	AccAuthenticator = radius:authenticator(),
	B0 = radius_attributes:new(),
	B1 = radius_attributes:add(?UserName, UserName, B0),
	B2 = radius_attributes:add(?UserPassword, Password, B1),
	B3 = radius_attributes:add(?NasIdentifier, NasId, B2),
	B4 = radius_attributes:add(?NasPortType, 19, B3),
	B5 = radius_attributes:add(?CallingStationId,"de:ad:be:ef:ca:fe", B4),
	B6 = radius_attributes:add(?AcctSessionId, AcctSessionId, B5),
	B7 = radius_attributes:add(?AcctStatusType, ?AccountingStart, B6),
	AccAttributes = radius_attributes:codec(B7),
	AccountingRequest = #radius{code = ?AccountingRequest, id = NextId,
			authenticator = AccAuthenticator, attributes = AccAttributes},
	AccPacket = radius:codec(AccountingRequest),
	ok = gen_udp:send(Socket, AcctAddress, AcctPort, AccPacket),
	{ok, {AcctAddress, AcctPort, AccResponse}} = gen_udp:recv(Socket, 0),
	ResponseLength = size(AccResponse),
	#radius{code = ?AccountingResponse, id = NextId,
			authenticator = ResponseAuthenticator} = radius:codec(AccResponse),
	ResponseAuthenticator = crypto:hash(md5, [<<?AccountingResponse, NextId,
			ResponseLength:16>>, AccAuthenticator, AccAttributes, SharedSecret]).
...
disconnect_session(Config) ->
	...
	IDReqAttributeList3 = radius_attributes:store(?NasPortType, 19, IDReqAttributeList2),
	IDReqAttributeList4 = radius_attributes:store(?CallingStationId,"de:ad:be:ef:ca:fe",
		IDReqAttributeList3),
	IDReqAttributeList5 = radius_attributes:store(?MessageAuthenticator, <<0:128>>, IDReqAttributeList4),
	...
	RADAcctAttributes5 = radius_attributes:store(?MessageAuthenticator, <<0:128>>, RADAcctAttributes4),
	...
	RADAcctAttributes18 = radius_attributes:store(?MessageAuthenticator, <<0:128>>, RADAcctAttributes17),
	...
	RADAcctAttributes28 = radius_attributes:store(?MessageAuthenticator, <<0:128>>, RADAcctAttributes27),
