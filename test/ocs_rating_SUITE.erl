...
initial_insufficient(_Config) ->
	...
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	ServiceType = 2,
	{out_of_credit, _} = ocs_rating:rate(radius, ServiceType, undefined,
...
interim_debit_and_reserve_charging_key(_Config) ->
	...
	#bucket{id = BId2,
			reservations = Reservations1} = lists:keyfind("session",
			#bucket.name, ocs:get_buckets(ProdRef)),
	2 = length(Reservations1),
	...
	{ok, #bucket{reservations = Reservations2}} = ocs:find_bucket(BId2),
	2 = length(Reservations2),
