...
initial_insufficient(_Config) ->
	...
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	ServiceType = 2,
	{out_of_credit, _} = ocs_rating:rate(radius, ServiceType, undefined, ServiceId,
...
initial_add_session(_Config) ->
	...
	{ok, #service{session_attributes = [{_, SessionId}]}, PackageSize} =
			ocs_rating:rate(radius, ServiceType, undefined, ServiceId, Timestamp,
...
initial_overhead(_Config) ->
	...
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	ServiceType = 2,
	{ok, #service{}, Reserved} = ocs_rating:rate(radius, ServiceType, undefined, ServiceId,
...
initial_expire_buckets(_Config) ->
	...
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	ServiceType = 32251,
	{out_of_credit, _} = ocs_rating:rate(diameter, ServiceType, undefined, ServiceId,
...
interim_reserve(_Config) ->
	...
	{ok, _Service, PackageSize} = ocs_rating:rate(radius, ServiceType,
			undefined, ServiceId, TS, undefined, undefined, initial, [],
	...
	{ok, #service{}, _Reservation} = ocs_rating:rate(radius, ServiceType,
			undefined, ServiceId, calendar:gregorian_seconds_to_datetime(TS + 60), undefined,
...
interim_reserve_within_unit_size(_Config) ->
	...
	%% 1st Reservation
	Reservation1 = 100,
	{ok, #service{}, PackageSize} = ocs_rating:rate(diameter, ServiceType, undefined, ServiceId,
...
interim_reserve_remove_session(_Config) ->
	...
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, undefined, ServiceId, TS,
...
interim_reserve_multiple_buckets_available(_Config) ->
	...
	Reservation2 = 1100,
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, undefined, ServiceId,
			calendar:gregorian_seconds_to_datetime(TS + 60), undefined,
...
interim_debit_out_of_credit(_Config) ->
	...
	TS = calendar:datetime_to_gregorian_seconds(calendar:local_time()),
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	{ok, _Service1, PackageSize} = ocs_rating:rate(diameter, ServiceType, undefined,
...
interim_debit_remove_session(_Config) ->
	...
	{out_of_credit, _} = ocs_rating:rate(diameter, ServiceType,
			undefined, ServiceId, Timestamp, undefined, undefined, interim,
...
final_remove_session(_Config) ->
	...
	{ok, _, _, _} = ocs_rating:rate(diameter, ServiceType, undefined, ServiceId,
			calendar:gregorian_seconds_to_datetime(TS + 60), undefined,
...
final_voice(_Config) ->
	...
	{ok, _, _} = ocs_rating:rate(radius, ServiceType, undefined, ServiceId,
	...
	{ok, _, _, _} = ocs_rating:rate(radius, ServiceType, undefined, ServiceId,
...
debit_sms(_Config) ->
	...
	[{_, _, Reserved, _}] = Rs1,
	Reserved = PackagePrice * NumOfEvents,
	{ok, _, _, Rated} = ocs_rating:rate(diameter, ServiceType, undefined, ServiceId,
