...
initial_exact_fit(_Config) ->
	...
	{ok, _, PackageSize} = ocs_rating:rate(diameter, ServiceType, undefined,
...
initial_insufficient(_Config) ->
	...
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	ServiceType = 2,
	{out_of_credit, _} = ocs_rating:rate(radius, ServiceType, undefined, undefined,
...
initial_add_session(_Config) ->
	...
	{ok, #service{session_attributes = [{_, SessionId}]}, PackageSize} =
			ocs_rating:rate(radius, ServiceType, undefined, undefined,
			ServiceId, Timestamp, undefined, undefined, initial, [],
...
initial_overhead(_Config) ->
	...
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	ServiceType = 2,
	{ok, #service{}, Reserved} = ocs_rating:rate(radius, ServiceType, undefined,
			undefined, ServiceId, Timestamp, undefined, undefined, initial, [],
			[{octets, Reservation}],
			SessionId),
...
initial_multiple_buckets(_Config) ->
	...
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	{ok, #service{}, Reserved} =
			ocs_rating:rate(radius, ServiceType, undefined, undefined, ServiceId,
...
initial_ignore_expired_buckets(_Config) ->
	...
	{ok, #service{}, PackageSize} = ocs_rating:rate(diameter, ServiceType,
			...
			initial, [], [{octets, Reservation}], SessionId2),
...
interim_reserve(_Config) ->
	...
	{ok, _Service, PackageSize} = ocs_rating:rate(radius, ServiceType,
			undefined, undefined, ServiceId, Timestamp, undefined, undefined,
			initial, [], [{octets, InitialReservation}], SessionId),
...
interim_reserve_out_of_credit(_Config) ->
	...
	{ok, _Service2, _} = ocs_rating:rate(diameter, ServiceType, undefined,
			...
			initial, [], [{octets, ReserveSize}], SessionId),
...
interim_reserve_remove_session(_Config) ->
	...
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, undefined,
			undefined, ServiceId, Timestamp, undefined, undefined,
			initial, [], [{octets, Reservation1}], SessionAttributes),
	Reservation2 = 1100,
...
interim_debit_under_unit_size(_Config) ->
	...
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, undefined,
			undefined, ServiceId, Timestamp, undefined,
			undefined, interim, [{octets, 1000000}], [], SessionId),
	{ok, #product{balance = BucketRefs}} = ocs:find_product(ProdRef),
	RatedBuckets = lists:flatten([mnesia:dirty_read(bucket, Id)
			|| Id <- BucketRefs]),
...
interim_debit_remove_session(_Config) ->
	...
	{out_of_credit, _} = ocs_rating:rate(diameter, ServiceType,
			undefined, undefined, ServiceId, Timestamp, undefined, undefined,
			interim, [{octets, Debit}], [], SessionId),
	{ok, #service{session_attributes = []}} = ocs:find_service(ServiceId).
...
interim_debit_and_reserve_available(_Config) ->
	...
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, undefined,
			undefined, ServiceId, Timestamp, undefined, undefined,
			initial, [], [{octets, Reservation}], SessionId),
	{ok, #product{balance = BucketRefs1}} = ocs:find_product(ProdRef),
	RatedBuckets1 = lists:flatten([mnesia:dirty_read(bucket, Id)
			|| Id <- BucketRefs1]),
...
interim_debit_and_reserve_insufficient2(_Config) ->
	...
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, undefined,
			undefined, ServiceId, Timestamp, undefined, undefined,
			initial, [], [{octets, UnitSize}], SessionId),
	{out_of_credit, _} = ocs_rating:rate(diameter, ServiceType,
			undefined, undefined, ServiceId,
			calendar:gregorian_seconds_to_datetime(TS + 60), undefined, undefined,
...
final_remove_session(_Config) ->
	P1 = price(usage, octets, 10000000, 100),
	OfferId = add_offer([P1], 4),
	ProdRef = add_product(OfferId),
	ServiceId =  add_service(ProdRef),
	RemAmount = 1000,
	...
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, undefined,
			undefined, ServiceId, Timestamp, undefined, undefined,
			initial, [], [], [SA2]),
	{ok, _, _, _} = ocs_rating:rate(diameter, ServiceType,
...
final_refund(_Config) ->
	...
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, undefined,
			undefined, ServiceId, Timestamp, undefined, undefined,
			initial, [], [{octets, UnitSize}], SessionId1),
	{ok, #product{balance = BucketRefs1}} = ocs:find_product(ProdRef),
	RatedBuckets1 = lists:flatten([mnesia:dirty_read(bucket, Id)
			|| Id <- BucketRefs1]),
...
final_voice(_Config) ->
	...
	{ok, _, _, _} = ocs_rating:rate(radius, ServiceType, undefined,
...
debit_sms(_Config) ->
	...
	[{_, _, Reserved, _}] = Rs1,
	Reserved = PackagePrice * NumOfEvents,
	{ok, _, _, Rated} = ocs_rating:rate(diameter, ServiceType, undefined,
			undefined, ServiceId, Timestamp, undefined, undefined,
...
roaming_table_data(Config) ->
	...
	{ok, _A, PackageSize}= ocs_rating:rate(diameter, ServiceType,
			undefined, SNPrefix, ServiceId, Timestamp, undefined, undefined,
			initial, [], [{octets, PackageSize}], SessionId),
	ok = file:close(File).
...
roaming_table_voice(Config) ->
	...
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	{ok, _A, PackageSize} = ocs_rating:rate(diameter, ServiceType,
			undefined, SNPrefix, ServiceId, Timestamp, ServiceId, undefined,
			initial, [], [{seconds, PackageSize}], SessionId),
	ok = file:close(File1),
	ok = file:close(File2).
...
roaming_table_sms(Config) ->
	...
	{ok, _A, PackageSize} = ocs_rating:rate(diameter, ServiceType,
			undefined, SNPrefix, ServiceId, Timestamp, ServiceId, undefined,
			initial, [], [{messages, PackageSize}], SessionId),
	ok = file:close(File1),
	ok = file:close(File2).
