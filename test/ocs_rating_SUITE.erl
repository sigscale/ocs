...
initial_exact_fit(_Config) ->
	...
	{ok, _Service1} = ocs:add_service(ServiceID, Password, ProdID, Chars, Buckets),
	{ok, Service2, PackageSize} = ocs_rating:rate(diameter, ServiceType,
			ServiceID, Timestamp, undefined, undefined, initial, [],
			[{octets, PackageSize}], SessionId),
	#service{buckets = [#bucket{remain_amount = 0,
			reservations = [{_, 0, PackagePrice, _}]}]} = Service2.
...
initial_insufficient(_Config) ->
	...
	{ok, #service{buckets = [#bucket{units = cents, remain_amount = RemAmount,
			reservations = []}]}} = ocs:find_service(ServiceID).
...
initial_insufficient_multisession(_Config) ->
	...
	{ok, #service{buckets = Buckets1}} =
		ocs:add_service(ServiceID, Password, ProdID, Chars, Buckets),
	{out_of_credit, _} = ocs_rating:rate(radius, ServiceType, ServiceID,
			Timestamp, undefined, undefined, initial, [], [{octets, PackageSize}], SessionId),
	{ok, #service{buckets = Buckets1}} = ocs:find_service(ServiceID).
...
initial_add_session(_Config) ->
	...
	{ok, _} = ocs:add_service(ServiceID, Password, ProdID, Chars, Buckets),
	{ok, #service{session_attributes = [{_, SessionId}], buckets = NewBuckets},
			PackageSize} = ocs_rating:rate(radius, ServiceType, ServiceID,
...
initial_overhead(_Config) ->
	...
	{ok, #service{buckets = [#bucket{remain_amount = RemAmount2,
			...
			ocs_rating:rate(radius, ServiceType, ServiceID,
			Timestamp, undefined, undefined, initial,
...
initial_expire_buckets(_Config) ->
	...
	{out_of_credit, _} = ocs_rating:rate(diameter, ServiceType, ServiceID,
			Timestamp, undefined, undefined, initial, [], [{octets, PackageSize}], SessionId),
	{ok, #service{buckets = []}} = ocs:find_service(ServiceID).
...
initial_ignore_expired_buckets(_Config) ->
	...
	{ok, _Service1} = ocs:add_service(ServiceID,
	...
	ServiceType = <<"32251@3gpp.org">>,
	{ok, #service{buckets = Buckets2},
			PackageSize} = ocs_rating:rate(diameter, ServiceType, ServiceID,
			Timestamp, undefined, undefined, initial,
...
initial_negative_balance(_Config) ->
	...
	Password = ocs:generate_password(),
	Chars = [{validity, erlang:system_time(?MILLISECOND) - 2592000000}],
	{ok, #service{buckets = Buckets1}} = ocs:add_service(ServiceID,
	...
	{out_of_credit, _} = ocs_rating:rate(diameter, ServiceType, ServiceID,
			Timestamp, undefined, undefined, initial,
			[], [{octets, Reservation}], SessionId),
	{ok, #service{buckets = Buckets2}} = ocs:find_service(ServiceID),
...
interim_reserve(_Config) ->
	...
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	ServiceType = 2,
	{ok, _Service2, PackageSize} = ocs_rating:rate(radius, ServiceType,
	...
	{ok, #service{buckets = [#bucket{remain_amount = 0,
			reservations = [{_, 0, Reserved, _}]}]},
			_Reservation} = ocs_rating:rate(radius, ServiceType, ServiceID,
...
interim_reserve_within_unit_size(_Config) ->
	...
	%% 1st Reservation
	Reservation1 = 100,
	{ok, #service{buckets = [#bucket{remain_amount = CentsRemain1,
			reservations = [{_, 0, Reserved1, _}]}]}, PackageSize}
			= ocs_rating:rate(diameter, ServiceType, ServiceID, TS,
			undefined, undefined, initial, [], [{octets, Reservation1}], SessionId),
	CentsRemain1 = RemAmount - F(Reservation1),
	Reserved1 = F(Reservation1),
	%% 2nd Reservation
	Reservation2 = 300,
	{ok, #service{buckets = [#bucket{remain_amount = CentsRemain2,
			reservations = [{_, 0, Reserved2, _}]}]}, PackageSize}
			= ocs_rating:rate(diameter, ServiceType, ServiceID,
	...
	%% 3rd Reservation
	Reservation3 = 700,
	{ok, #service{buckets = [#bucket{remain_amount = CentsRemain3,
...
interim_reserve_out_of_credit(_Config) ->
	...
	{out_of_credit, _} = ocs_rating:rate(diameter, ServiceType, ServiceID,
			...
			[{octets, ReserveSize}], SessionId),
	{ok, #service{buckets = RatedBuckets}} = ocs:find_service(ServiceID),
...
interim_reserve_remove_session() ->
	[{userdata, [{doc, "Out of credit remove session attributes from service record"}]}].
...
interim_reserve_remove_session(_Config) ->
	...
	Buckets = [#bucket{units = cents, remain_amount = RemAmount,
		start_date = erlang:system_time(?MILLISECOND),
		termination_date = erlang:system_time(?MILLISECOND) + 2592000000}],
	Service = #service{name = ServiceID, password = Password,
	...
	ServiceType = <<"32251@3gpp.org">>,
	ok = mnesia:dirty_write(service, Service),
...
interim_reserve_multiple_buckets_available(_Config) ->
	...
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, ServiceID, TS,
			undefined, undefined, initial, [], [{octets, Reservation1}], SessionAttributes),
	{ok, #service{buckets = RatedBuckets1}} = ocs:find_service(ServiceID),
	...
	{ok, #service{buckets = RatedBuckets2}} = ocs:find_service(ServiceID),
...
interim_debit_out_of_credit(_Config) ->
	...
	{ok, _Service2, PackageSize} = ocs_rating:rate(diameter, ServiceType,
			ServiceID, TS, undefined, undefined, initial,
			[], [{octets, PackageSize}], SessionId),
	{ok, _Service3, PackageSize} = ocs_rating:rate(diameter, ServiceType,
			ServiceID, calendar:gregorian_seconds_to_datetime(TS + 60),
...
interim_out_of_credit_voice(_Config) ->
	...
	Buckets = [#bucket{units = cents, remain_amount = StartingAmount,
		start_date = erlang:system_time(?MILLISECOND),
		termination_date = erlang:system_time(?MILLISECOND) + 2592000000}],
	{ok, _} = ocs:add_service(ServiceID, Password, ProdID, [], Buckets),
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
...
final_remove_session() ->
	[{userdata, [{doc, "Final call remove session attributes from service record"}]}].
...
final_refund(_Config) ->
	...
	{ok, #service{buckets = RatedBuckets3}} = ocs:find_service(ServiceID),
...
final_voice(_Config) ->
	ProductID = ocs:generate_password(),
	UnitPrice = 5,
	UnitSize = 60,
	ReserveTime = 300,
	...
	Product = #product{name = ProductID, price = [Price], specification = "9"},
	{ok, _} = ocs:add_product(Product),
	ServiceID = list_to_binary(ocs:generate_identity()),
	Password = ocs:generate_password(),
	StartingAmount = 305,
	...
	{ok, _S} = ocs:add_service(ServiceID,
...
reserve_data(_Config) ->
	...
	Buckets = [#bucket{units = cents, remain_amount = StartingAmount,
			start_date = erlang:system_time(?MILLISECOND),
			termination_date = erlang:system_time(?MILLISECOND) + 2592000000}],
	{ok, _} = ocs:add_service(ServiceID,
...
authorize_voice(_Config) ->
	...
	SessionId = [{?AcctSessionId, list_to_binary(ocs:generate_password())}],
	ServiceType = 12,
	{ok, _S} = ocs:add_service(ServiceID, Password, ProdID, Chars, Buckets),
