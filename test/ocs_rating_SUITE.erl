...
initial_negative_balance(_Config) ->
	...
	Price2 = #price{name = "usage", type = usage,
			units = octets, size = PackageSize, amount = PackagePrice},
	Product = #offer{name = ProdID, price = [Price1, Price2],
			specification = 4},
	{ok, _} = ocs:add_offer(Product),
	SubscriberID = list_to_binary(ocs:generate_identity()),
	Timestamp = calendar:local_time(),
...
interim_reserve_out_of_credit(_Config) ->
	...
	Product1 = #offer{name = ProdID, price = [Price], specification = 4},
	{ok, _Product2} = ocs:add_offer(Product1),
	SubscriberID = list_to_binary(ocs:generate_identity()),
	TS = calendar:datetime_to_gregorian_seconds(calendar:local_time()),
	Password = ocs:generate_password(),
	Chars = [{validity, erlang:system_time(?MILLISECOND) + 2592000000}],
...
interim_reserve_remove_session(_Config) ->
	...
	Buckets = [#bucket{units = cents, remain_amount = RemAmount,
		start_date = erlang:system_time(?MILLISECOND),
		termination_date = erlang:system_time(?MILLISECOND) + 2592000000}],
	Subscriber = #service{name = SubscriberID, password = Password,
...
interim_reserve_multiple_buckets_out_of_credit(_Config) ->
	...
	B2 = #bucket{units = cents, remain_amount = StartAmount2,
		...
		termination_date = erlang:system_time(?MILLISECOND) + 2592000000},
	Buckets = [B1, B2],
	Subscriber = #service{name = SubscriberID,
...
interim_debit_remove_session(_Config) ->
	...
	mnesia:dirty_write(service, Subscriber),
	SessionId = [{'Session-Id', list_to_binary(ocs:generate_password())}],
	ServiceType = <<"32251@3gpp.org">>,
	{out_of_credit, _} = ocs_rating:rate(diameter, ServiceType,
...
interim_out_of_credit_voice(_Config) ->
	...
	Product = #offer{name = ProdID, price = [Price], specification = 9},
	{ok, _} = ocs:add_offer(Product),
	SubscriberID = list_to_binary(ocs:generate_identity()),
	TS = calendar:datetime_to_gregorian_seconds(calendar:local_time()),
	Password = ocs:generate_password(),
...
final_remove_session(_Config) ->
	...
	{ok, _, _, _} = ocs_rating:rate(diameter, ServiceType, SubscriberID,
			calendar:gregorian_seconds_to_datetime(TS + 60),
...
final_voice(_Config) ->
	...
	Product = #offer{name = ProductID, price = [Price], specification = "9"},
	{ok, _} = ocs:add_offer(Product),
	SubscriberID = list_to_binary(ocs:generate_identity()),
	Password = ocs:generate_password(),
	StartingAmount = 305,
	...
	{ok, _, _, _} = ocs_rating:rate(radius, ServiceType, SubscriberID,
...
reserve_data(_Config) ->
	...
	DataProduct = #offer{name = DataProdID, price = [DataPrice],
			specification = "8"},
	{ok, _} = ocs:add_offer(DataProduct),
	...
	VoicePrice = #price{name = "Calls", type = usage,
			...
			min = 1, max = 1, values = [#char_value{default = true,
			units = seconds, value = ReserveTime}]}]},
	VoiceProduct = #offer{name = VoiceProdID, price = [VoicePrice],
			specification = "9"},
	{ok, _} = ocs:add_offer(VoiceProduct),
	BundleProdID = ocs:generate_password(),
	BundleProduct = #offer{name = BundleProdID,
...
reserve_incoming_voice(_Config) ->
	...
	VoicePrice2 = #price{name = "Incoming Calls", type = usage,
			...
			min = 1, max = 1, values = [#char_value{default = true,
			value = "answer"}]}]},
	VoiceProduct = #offer{name = VoiceProdID,
...
time_of_day(_Config) ->
	...
	DataProduct = #offer{name = DataProdID,
...
reserve_sms(_Config) ->
	...
	{ok, _, _} = ocs_rating:rate(diameter, ServiceType, SubscriberID,
		...
		SessionId),
	{ok, #service{buckets = [B1]}} = ocs:find_subscriber(SubscriberID),
...
debit_sms(_Config) ->
	...
	{ok, _, _, Rated} = ocs_rating:rate(diameter, ServiceType, SubscriberID,
		...
		SessionId),
	{ok, #service{buckets = [B2]}} = ocs:find_subscriber(SubscriberID),
