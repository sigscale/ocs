...
start12() ->
	...
	start13(Tables, mnesia:wait_for_tables(Tables, ?WAITFORTABLES)).
%% @hidden
start13(_Tables, ok) ->
	start14();
start13(_Tables, {timeout, BadTables}) ->
	F = fun F([Table | T]) ->
...
start17(Sup) ->
	try ocs_mib:load() of
		ok ->
			start18(Sup);
		{error, _Reason} ->
			error_logger:warning_msg("OCS SNMP MIB not loaded"),
			start18(Sup)
	catch
		_:_Reason ->
			error_logger:warning_msg("OCS SNMP MIB not loaded"),
