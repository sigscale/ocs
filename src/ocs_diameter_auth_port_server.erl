...
handle_call({diameter_request, Caps, ClientAddr, ClientPort,
		PasswordReq, Trusted, Request, Eap} = _Request,
		CbProc, State) ->
...
%% @private
%%
handle_cast({AuthFsm, AuthRequest} = _Request,
		#state{cb_fsms = CbHandler} = State) ->
	case gb_trees:lookup(AuthFsm, CbHandler) of
		{value, CbProc} ->
			gen_server:reply(CbProc, AuthRequest),
...
handle_info(timeout = _Info,
	#state{simple_auth_sup = undefined,
			pwd_sup = undefined, ttls_sup = undefined,
			aka_sup = undefined, akap_sup = undefined} = State) ->
	init1(State);
...
handle_info({'EXIT', Pid, _Reason},
		#state{handlers = Handlers} = State) ->
	Fdel = fun F({Key, Fsm, _Iter}) when Fsm == Pid ->
				Key;
			F({_, _, Iter}) ->
