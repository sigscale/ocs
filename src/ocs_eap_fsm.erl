...
idle(timeout, #statedata{identifier = Identifier, radius_fsm = RadiusFsm } = StateData) ->
	...
	GeneralPacket = ocs_eap_codec:eap_packet(Packet),
	Header = #eap_pwd{type = ?PWD, length = false, more = false, pwd_exch = 16#1,
		data = IDReqBody},
	IDReqHeader = ocs_eap_codec:eap_pwd(Header),
	IDRequest = <<GeneralPacket/binary, IDReqHeader/binary, IDReqBody/binary>>,
