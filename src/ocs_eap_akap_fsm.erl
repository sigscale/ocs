...
%%% 	module implements the functions associated with an EAP server within
%%% 	EAP 3rd Generation Authentication and Key Agreement (EAP-AKA')
...
-export([eap_start/3, identity/3, vector/3, challenge/3,
			register/3, failure/3]).
...
-type state() :: eap_start | identity | vector
		| challenge | register | failure.
...
%%----------------------------------------------------------------------
%%  The ocs_eap_akap_fsm gen_statem call backs
%%----------------------------------------------------------------------
...
init([Sup, diameter, ServerAddress, ServerPort, ClientAddress, ClientPort,
	...
	case global:whereis_name({ocs_diameter_auth,
		...
		PortServer ->
			...
			{ok, eap_start, Data, {next_event, internal, start}}
		end;
...
eap_start(internal = _EventType, start = _EventContent,
		...
		trusted = Trusted, keys = Keys} = Data) ->
	Children = supervisor:which_children(Sup),
	{_, AucFsm, _, _} = lists:keyfind(ocs_eap_aka_auc_fsm, 1, Children),
	NewData = Data#statedata{auc_fsm = AucFsm},
	...
	case radius_attributes:find(?EAPMessage, RequestAttributes) of
		{ok, <<>>} ->
			...
			NextData = NewData#statedata{request = undefined, id_req = full},
			...
			send_radius_response(EapMessage1, ?AccessChallenge,
					...
					RequestAttributes, undefined, NextData),
			{next_state, identity, NextData, Action};
		{ok, EapMessage1} ->
			try
				case ocs_eap_codec:eap_packet(EapMessage1) of
					#eap_packet{code = response, type = ?Identity,
						...
						NextData = NewData#statedata{eap_id = StartEapID,
								imsi = IMSI, identity = Identity},
						% @todo handle RADIUS attribute for ANID
						gen_statem:cast(AucFsm, {vector, {self(), IMSI, undefined,
								get_radius_rat(RequestAttributes), "WLAN"}}),
						{next_state, vector, NextData, Action};
					#eap_packet{code = response, type = ?Identity,
							identifier = StartEapID,
					...
					#eap_packet{code = response, type = ?Identity,
						...
						EapData = ocs_eap_codec:eap_aka(#eap_aka_identity{fullauth_id_req = true}),
						NextData = NewData#statedata{request = undefined,
					...
					#eap_packet{code = request, identifier = StartEapID} ->
						...
						send_radius_response(EapMessage2, ?AccessReject,
								[], RadiusID, RequestAuthenticator,
								RequestAttributes, undefined, NewData),
						{stop, {shutdown, SessionId}};
					#eap_packet{code = Code, type = EapType,
							identifier = StartEapID, data = EapData} ->
...
eap_start(internal = _EventType, start = _EventContent,
		...
		diameter_port_server = PortServer, sup = Sup} = Data) ->
	Children = supervisor:which_children(Sup),
	{_, AucFsm, _, _} = lists:keyfind(ocs_eap_aka_auc_fsm, 1, Children),
	EapData = ocs_eap_codec:eap_aka(#eap_aka_identity{fullauth_id_req = true}),
	...
	send_diameter_response(SessionId, AuthReqType,
			...
			EapMessage, PortServer, Request, undefined, NewData),
	Action = {timeout, ?TIMEOUT, timeout},
	{next_state, identity, NewData, Action};
eap_start(internal = _EventType, start = _EventContent,
		#statedata{eap_id = EapID,
		session_id = SessionId, auth_req_type = AuthReqType,
	...
	{next_state, identity, NewData, Action};
eap_start(internal = _EventType, start = _EventContent,
		#statedata{request = #diameter_eap_app_DER{'EAP-Payload' = EapMessage,
		'NAS-Port-Type' = NasPortType}} = Data) ->
	eap_start1(EapMessage, nas_port_type(NasPortType), Data);
eap_start(internal = _EventType, start = _EventContent,
		#statedata{request = #'3gpp_sta_DER'{'EAP-Payload' = EapMessage,
		'RAT-Type' = [RAT]}} = Data) ->
	eap_start1(EapMessage, RAT, Data).
%% @hidden
	...
	try
		case ocs_eap_codec:eap_packet(EapMessage) of
			#eap_packet{code = response, type = ?Identity,
				...
				gen_statem:cast(AucFsm, {vector, {self(),
			...
			#eap_packet{code = response, type = ?Identity,
				...
				send_diameter_response(SessionId, AuthReqType,
						?'DIAMETER_BASE_RESULT-CODE_MULTI_ROUND_AUTH',
						EapMessage1, PortServer, Request, undefined, NextData),
				{next_state, identity, NextData, Action};
			#eap_packet{code = response, type = ?Identity,
					identifier = StartEapID, data = _Identity} ->
				NextEapID = (StartEapID rem 255) + 1,
			...
			#eap_packet{code = request, identifier = NewEapID} ->
				...
				send_diameter_response(SessionId, AuthReqType,
						...
						EapMessage1, PortServer, Request, undefined, NewData),
		...
		{'EXIT', Reason} ->
			...
			send_diameter_response(SessionId, AuthReqType,
					?'DIAMETER_BASE_RESULT-CODE_UNABLE_TO_COMPLY',
					EapMessage2, PortServer, Request, undefined, NewData),
			{stop, {shutdown, SessionId}, NewData}
	end.

-spec identity(EventType, EventContent, Data) -> Result
	when
		EventType :: gen_statem:event_type(),
		EventContent :: term(),
		Data :: statedata(),
		Result :: gen_statem:event_handler_result(state()).
%% @doc Handles events received in the <em>identity</em> state.
%% @@see //stdlib/gen_statem:StateName/3
%% @private
%%
identity(timeout = _EventType, _EventContent,
		#statedata{session_id = SessionId} = _Data) ->
	{stop, {shutdown, SessionId}};
identity(cast = _EventType,
		...
		auc_fsm = AucFsm, keys = Keys, id_req = IdReq} = Data) ->
	...
	try
			...
			#eap_packet{code = response, type = ?AKAprime,
				...
				case ocs_eap_codec:eap_aka(EapData) of
					...
					#eap_aka_identity{identity = <<?TEMP_AKAp:6, _/bits>> = Identity}
						...
						gen_statem:cast(AucFsm, {vector, {self(), IMSI,
								undefined, get_radius_rat(RequestAttributes), "WLAN"}}),
						NextData = NewData#statedata{identity = Identity,
								imsi = IMSI},
						{next_state, vector, NextData, Action}
				end;
		...
		_:Reason ->
			...
			EapData1 = ocs_eap_codec:eap_aka(Notification),
			EapPacket = #eap_packet{code = request,
					type = ?AKAprime, identifier = EapID, data = EapData1},
			EapMessage2 = ocs_eap_codec:eap_packet(EapPacket),
			NextData1 = NewData#statedata{failure = ?AccessReject},
			send_radius_response(EapMessage2, ?AccessChallenge,
					[], RadiusID, RequestAuthenticator,
					RequestAttributes, undefined,  NextData1),
			{next_state, failure, NextData1, Action}
	end;
identity(cast = _EventType,
		#diameter_eap_app_DER{'EAP-Payload' = EapMessage,
				'NAS-Port-Type' = NasPortType} = Request, Data) ->
	identity1(EapMessage, nas_port_type(NasPortType),
			Data#statedata{request = Request});
identity(cast = _EventType,
		#'3gpp_sta_DER'{'EAP-Payload' = EapMessage,
				'RAT-Type' = [RAT]} = Request, Data) ->
	identity1(EapMessage, RAT, Data#statedata{request = Request}).
%% @hidden
identity1(EapMessage, RAT,
		...
		diameter_port_server = PortServer, keys = Keys} = Data) ->
	Action = {timeout, ?TIMEOUT, timeout},
	try
		case ocs_eap_codec:eap_packet(EapMessage) of
			#eap_packet{code = response, type = ?AKAprime,
				...
				case ocs_eap_codec:eap_aka(EapData) of
					#eap_aka_identity{identity = <<?PERM_AKAp,
						...
						gen_statem:cast(AucFsm, {vector, {self(), IMSI,
								undefined, RAT, "WLAN"}}),
						NewData = Data#statedata{identity = Identity,
								imsi = IMSI},
						{next_state, vector, NewData, Action};
					#eap_aka_identity{identity = <<?TEMP_AKAp:6, _/bits>> = Identity}
							when IdReq == full ->
						[Pseudonym | _] = binary:split(Identity, <<$@>>, []),
						CompressedIMSI = ocs_eap_aka:decrypt_imsi(Pseudonym, Keys),
		...
		_:Reason ->
			...
			EapPacket = #eap_packet{code = request,
					type = ?AKAprime, identifier = EapID, data = EapData1},
			EapMessage1 = ocs_eap_codec:eap_packet(EapPacket),
			NewData1 = Data#statedata{failure = ?'DIAMETER_BASE_RESULT-CODE_UNABLE_TO_COMPLY'},
			send_diameter_response(SessionId, AuthReqType,
					?'DIAMETER_BASE_RESULT-CODE_MULTI_ROUND_AUTH',
					EapMessage1, PortServer, Request, undefined, NewData1),
			{next_state, failure, NewData1, Action}
	end.
...
%% @private
%%
vector(timeout = _EventType, _EventContent,
		#statedata{session_id = SessionId} = _Data) ->
	{stop, {shutdown, SessionId}};
vector(cast = _EventType, {ok, {RAND, AUTN, CKprime, IKprime, XRES}},
		#statedata{eap_id = EapID,
		request = #radius{code = ?AccessRequest, id = RadiusID,
	...
	send_radius_response(EapMessage2, ?AccessChallenge,
			[], RadiusID, RequestAuthenticator,
			RequestAttributes, undefined, NewData),
	{next_state, challenge, NewData};
vector(cast = _EventType, {ok, {RAND, AUTN, CKprime, IKprime, XRES}},
		#statedata{eap_id = EapID,
		request = Request, auth_req_type = AuthReqType,
		diameter_port_server = PortServer,
		session_id = SessionId, identity = Identity} = Data) ->
	NextEapID = (EapID rem 255) + 1,
	<<Kencr:16/binary, Kaut:32/binary, Kre:32/binary, MSK:64/binary,
			EMSK:64/binary, _/binary>> = prf(<<IKprime/binary,
			CKprime/binary>>, <<"EAP-AKA'", Identity/binary>>, 7),
	...
	send_diameter_response(SessionId, AuthReqType,
			...
			EapMessage2, PortServer, Request, undefined, NewData),
	{next_state, challenge, NewData};
vector(cast = _EventType, {error, _Reason},
		#statedata{eap_id = EapID,
		request = #radius{code = ?AccessRequest, id = RadiusID,
		authenticator = RequestAuthenticator,
		attributes = RequestAttributes}} = Data) ->
	Notification = #eap_aka_notification{notification = 16384},
	EapData1 = ocs_eap_codec:eap_aka(Notification),
	EapPacket = #eap_packet{code = request,
			type = ?AKAprime, identifier = EapID, data = EapData1},
	EapMessage = ocs_eap_codec:eap_packet(EapPacket),
	NewData = Data#statedata{failure = ?AccessReject},
	send_radius_response(EapMessage, ?AccessChallenge,
			[], RadiusID, RequestAuthenticator,
			RequestAttributes, undefined, NewData),
	Action = {timeout, ?TIMEOUT, timeout},
	{next_state, failure, NewData, Action};
vector(cast = _EventType, {error, user_unknown},
	...
	EapPacket = #eap_packet{code = request,
			type = ?AKAprime, identifier = EapID, data = EapData1},
	EapMessage = ocs_eap_codec:eap_packet(EapPacket),
	NewData = Data#statedata{failure = ?'DIAMETER_ERROR_USER_UNKNOWN'},
	send_diameter_response(SessionId, AuthReqType,
			?'DIAMETER_BASE_RESULT-CODE_MULTI_ROUND_AUTH',
			EapMessage, PortServer, Request, undefined, NewData),
	Action = {timeout, ?TIMEOUT, timeout},
...
vector(cast = _EventType, {error, _Reason},
	...
	EapPacket = #eap_packet{code = request,
			type = ?AKAprime, identifier = EapID, data = EapData1},
	EapMessage = ocs_eap_codec:eap_packet(EapPacket),
	NewData = Data#statedata{failure = ?'DIAMETER_BASE_RESULT-CODE_UNABLE_TO_COMPLY'},
	send_diameter_response(SessionId, AuthReqType,
			?'DIAMETER_BASE_RESULT-CODE_MULTI_ROUND_AUTH',
			EapMessage, PortServer, Request, undefined, NewData),
	Action = {timeout, ?TIMEOUT, timeout},
...
-spec challenge(EventType, EventContent, Data) -> Result
	when
		EventType :: gen_statem:event_type(),
		EventContent :: term(),
		Data :: statedata(),
		Result :: gen_statem:event_handler_result(state()).
%% @doc Handles events received in the <em>challenge</em> state.
%% @@see //stdlib/gen_statem:StateName/3
%% @private
%%
challenge(timeout = _EventType, _EventContent,
		#statedata{session_id = SessionId} = _Data) ->
	{stop, {shutdown, SessionId}};
challenge(cast = _EventType,
		{#radius{id = RadiusID, authenticator = RequestAuthenticator,
				attributes = RequestAttributes} = Request, RadiusFsm},
		#statedata{eap_id = EapID, session_id = SessionId,
		secret = Secret, auc_fsm = AucFsm,
		identity = <<?PERM_AKAp, _PermanentID/binary>> = Identity,
		imsi = IMSI, msk = MSK, res = RES, kaut = Kaut} = Data) ->
	NewData = Data#statedata{request = Request,
			radius_fsm = RadiusFsm},
	Action = {timeout, ?TIMEOUT, timeout},
	try
		...
		case ocs_eap_codec:eap_aka(EapData1) of
			#eap_aka_challenge{res = RES, mac = MAC} ->
				...
				case ?HMACN_SHA(Kaut, EapMessage2) of
					MAC ->
						...
						NextData = NewData#statedata{response = Response},
						gen_statem:cast(AucFsm, {register, {self(), IMSI}}),
						{next_state, register, NextData, Action};
					_MAC ->
						Notification = #eap_aka_notification{notification = 16384},
						EapData2 = ocs_eap_codec:eap_aka(Notification),
						EapPacket1 = #eap_packet{code = request,
								type = ?AKAprime, identifier = EapID, data = EapData2},
						EapMessage3 = ocs_eap_codec:eap_packet(EapPacket1),
						NextData = NewData#statedata{failure = ?AccessReject},
						send_radius_response(EapMessage3, ?AccessChallenge,
								[], RadiusID, RequestAuthenticator,
								RequestAttributes, undefined, NextData),
						{next_state, failure, NextData, Action}
				end;
			#eap_aka_challenge{} ->
				Notification = #eap_aka_notification{notification = 16384},
				EapData2 = ocs_eap_codec:eap_aka(Notification),
			...
			#eap_aka_synchronization_failure{auts = AUTS, kdf = [1]} = _EAP ->
				gen_statem:cast(AucFsm, {vector, {self(), IMSI, AUTS,
						get_radius_rat(RequestAttributes), "WLAN"}}),
				{next_state, vector, NewData, Action};
		...
		_:Reason ->
			...
			EapData3 = ocs_eap_codec:eap_aka(Notification1),
			EapPacket2 = #eap_packet{code = request,
					type = ?AKAprime, identifier = EapID, data = EapData3},
...
challenge(cast = _EventType,
		#diameter_eap_app_DER{'EAP-Payload' = EapMessage,
				'NAS-Port-Type' = NasPortType} = Request, Data) ->
	challenge1(EapMessage, Request, nas_port_type(NasPortType), Data);
challenge(cast = _EventType,
		#'3gpp_sta_DER'{'EAP-Payload' = EapMessage,
				'RAT-Type' = [RATType]} = Request, Data) ->
	challenge1(EapMessage, Request, RATType,Data);
challenge(cast = _EventType,
		#'3gpp_sta_DER'{'EAP-Payload' = EapMessage,
				'RAT-Type' = []} = Request, Data) ->
	challenge1(EapMessage, Request, 1, Data).
%% @hidden
challenge1(EapMessage1, Request, RAT,
		...
		imsi = IMSI, auc_fsm = AucFsm, res = RES, kaut = Kaut} = Data) ->
	NewData = Data#statedata{request = Request},
	Action = {timeout, ?TIMEOUT, timeout},
	try
		#eap_packet{code = response, type = ?AKAprime, identifier = EapID,
					...
					MAC ->
						...
						case Request#'3gpp_sta_DER'.'Service-Selection' of
							[] ->
								gen_statem:cast(AucFsm, {register, {self(), IMSI}});
							[APN] ->
								gen_statem:cast(AucFsm, {register, {self(), IMSI, APN}})
						end,
						{next_state, register, NextData};
					_MAC ->
						Notification = #eap_aka_notification{notification = 16384},
						EapData2 = ocs_eap_codec:eap_aka(Notification),
						EapPacket1 = #eap_packet{code = request,
								type = ?AKAprime, identifier = EapID, data = EapData2},
						EapMessage3 = ocs_eap_codec:eap_packet(EapPacket1),
						NextData = NewData#statedata{failure = ?'DIAMETER_BASE_RESULT-CODE_UNABLE_TO_COMPLY'},
						send_diameter_response(SessionId, AuthReqType,
								?'DIAMETER_BASE_RESULT-CODE_MULTI_ROUND_AUTH',
								EapMessage3, PortServer, Request, undefined, NextData),
						{next_state, failure, NextData, Action}
				end;
			#eap_aka_challenge{} ->
				Notification = #eap_aka_notification{notification = 16384},
				...
				{next_state, failure, NextData, Action};
			#eap_aka_synchronization_failure{auts = AUTS, kdf = [1]} = _EAP ->
				gen_statem:cast(AucFsm, {vector, {self(), IMSI, AUTS, RAT, "WLAN"}}),
				{next_state, vector, NewData, Action};
			#eap_aka_authentication_reject{} = _EAP ->
				Notification = #eap_aka_notification{notification = 16384},
				EapData2 = ocs_eap_codec:eap_aka(Notification),
		...
		_:Reason ->
			...
			send_diameter_response(SessionId, AuthReqType,
					?'DIAMETER_BASE_RESULT-CODE_MULTI_ROUND_AUTH',
					EapMessage4, PortServer, Request, undefined, NewData),
			{next_state, failure, NewData, Action}
	end.
	...
	case mnesia:transaction(F) of
		{atomic, ok} ->
			send_radius_response(EapMessage, ?AccessAccept,
					...
					RequestAttributes, UserProfile, Data),
			{stop, {shutdown, SessionId}};
		{aborted, Reason} ->
			{stop, Reason}
	end;
	...
	case mnesia:transaction(F) of
		{atomic, ok} ->
			send_diameter_response(SessionId, AuthReqType,
					...
					EapMessage, PortServer, Request, UserProfile, Data),
			{stop, {shutdown, SessionId}};
		{aborted, Reason} ->
			{stop, Reason}
	end;
register(cast = _EventType, {error, _Reason},
		#statedata{eap_id = EapID,
		request = #radius{code = ?AccessRequest, id = RadiusID,
		authenticator = RequestAuthenticator,
		attributes = RequestAttributes}} = Data) ->
	Notification = #eap_aka_notification{notification = 16384},
	EapData = ocs_eap_codec:eap_aka(Notification),
	EapPacket = #eap_packet{code = request,
			type = ?AKAprime, identifier = EapID, data = EapData},
	EapMessage = ocs_eap_codec:eap_packet(EapPacket),
	NewData = Data#statedata{failure = ?AccessReject},
...
-spec failure(EventType, EventContent, Data) -> Result
	when
		EventType :: gen_statem:event_type(),
		EventContent :: term(),
		Data :: statedata(),
		Result :: gen_statem:event_handler_result(state()).
%% @doc Handles events received in the <em>failure</em> state.
%% @@see //stdlib/gen_statem:StateName/3
%% @private
%%
failure(timeout = _EventType, _EventContent,
		#statedata{session_id = SessionId} = _Data) ->
	{stop, {shutdown, SessionId}};
failure(cast = _EventType,
		{#radius{id = RadiusID, authenticator = RequestAuthenticator,
				attributes = RequestAttributes} = Request, RadiusFsm},
		#statedata{eap_id = EapID, session_id = SessionId,
				failure = RadiusCode} = Data) ->
	NewData = Data#statedata{request = Request,
			radius_fsm = RadiusFsm},
	try
		EapMessage1 = radius_attributes:fetch(?EAPMessage, RequestAttributes),
		#eap_packet{code = response, type = ?AKAprime, identifier = EapID,
				data = EapData} = ocs_eap_codec:eap_packet(EapMessage1),
		case ocs_eap_codec:eap_aka(EapData) of
...
failure(cast = _EventType,
		#diameter_eap_app_DER{'EAP-Payload' = EapMessage} = Request,
		Data) ->
	failure1(EapMessage, Request, Data);
failure(cast = _EventType,
		#'3gpp_sta_DER'{'EAP-Payload' = EapMessage} = Request,
		Data) ->
	failure1(EapMessage, Request, Data).
%% @hidden
failure1(EapMessage1, Request, #statedata{eap_id = EapID,
		session_id = SessionId, auth_req_type = AuthReqType,
		origin_host = OHost, origin_realm = ORealm,
		diameter_port_server = PortServer,
		failure = ResultCode} = Data) ->
	try
		#eap_packet{code = response, type = ?AKAprime, identifier = EapID,
				data = EapData} = ocs_eap_codec:eap_packet(EapMessage1),
		case ocs_eap_codec:eap_aka(EapData) of
			#eap_aka_notification{mac = undefined} = _EAP ->
				...
				send_diameter_response(SessionId, AuthReqType, ResultCode,
						EapMessage2, PortServer, Request, undefined, Data),
				{stop, {shutdown, SessionId}}
			% #eap_aka_notification{mac = MAC} = _EAP ->
			% @todo Handle notification when P=0
		end
		...
		_:Reason ->
			...
			send_diameter_response(SessionId, AuthReqType,
					?'DIAMETER_BASE_RESULT-CODE_UNABLE_TO_COMPLY',
					EapMessage3, PortServer, Request, undefined, Data),
			{stop, {shutdown, SessionId}}
	end.
...
-spec send_diameter_response(SessionId, AuthType, ResultCode,
		EapMessage, PortServer, Request, UserProfile, Data) -> ok
	when
...
send_diameter_response(SessionId, AuthType,
		?'DIAMETER_BASE_RESULT-CODE_MULTI_ROUND_AUTH', EapMessage,
		PortServer, #'3gpp_sta_DER'{} = Request, undefined,
		#statedata{server_address = ServerAddress, server_port = ServerPort,
		client_address = ClientAddress, client_port = ClientPort,
		origin_host = OriginHost, origin_realm = OriginRealm,
		identity = Identity} = _Data)
...
send_diameter_response(SessionId, AuthType,
		...
		client_address = ClientAddress, client_port = ClientPort,
		origin_host = OriginHost, origin_realm = OriginRealm,
		msk = MSK, identity = Identity} = _Data) when
...
send_diameter_response(SessionId, AuthType,
		...
		#'3gpp_swx_Non-3GPP-User-Data'{'APN-OI-Replacement' = APNOIReplacement,
				'Context-Identifier' = [_ContextIdentifier],
